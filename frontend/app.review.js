...
async function hydrate() {
  ...
  if (!state.reviewCameraId && state.cameras[0]) {
    state.reviewCameraId = state.cameras[0].id;
  }
...
async function loadRecordings() {
  if (!state.reviewCameraId) {
    state.recordings = [];
    return;
  }
  state.recordings = await request(`/recordings?camera_id=${state.reviewCameraId}`);
}
...
function renderReview() {
  ...
  if (Number($("rangeEnd").value) === 0 || Number($("rangeEnd").value) > total) {
    $("rangeEnd").value = String(total);
  }
  if (Number($("rangeStart").value) > total) {
    $("rangeStart").value = "0";
  }
  ...
  if (state.recordings.length && !$("reviewVideo").src) {
    playVirtualSecond(0);
  }
}
...
function renderTimeline() {
  ...
  timeline.innerHTML = state.recordings
    .map((chunk, index) => {
      const start = new Date(chunk.start_time).toLocaleTimeString();
      return `<button type="button" class="chunk" data-chunk="${index}">${start}</button>`;
    })
    .join("");
...
function playVirtualSecond(second) {
  ...
  if (chunk) {
    playChunk(chunk.index, chunk.offset);
  }
}
...
function chunkAtSecond(second) {
  ...
  for (let index = 0; index < state.recordings.length; index += 1) {
    ...
    if (second >= cursor && second <= next) {
      return { chunk, index, offset: second - cursor };
    }
    cursor = next;
  }
  return null;
...
function updateSelectedRange() {
  ...
  if (!selected) {
    $("selectedRange").textContent = "Sin grabaciones disponibles.";
    return;
  }
  $("selectedRange").textContent = `Rango: ${selected.startTime.toLocaleString()} a ${selected.endTime.toLocaleString()} (${Math.round(
...
$("reviewVideo").addEventListener("ended", () => {
  ...
  if (index >= 0 && index < state.recordings.length - 1) {
    playChunk(index + 1, 0);
  }
});
