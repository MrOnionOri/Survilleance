<script>
  ...
  const eventTypes = ["normal", "freeze", "black_screen", "buffering", "ad", "repeated_ad", "scene_change"];
  ...
  $: criticalCount = events.filter((event) => ["freeze", "black_screen", "repeated_ad"].includes(event.type)).length;
  ...
  async function addCamera() {
    ...
    try {
      await api("/cameras", {
        method: "POST",
        body: JSON.stringify({ name: cameraName, source: cameraSource, enabled: true }),
      });
      cameraName = "";
      cameraSource = "";
      await hydrate();
  ...
  async function trainModel() {
    if (!canTrain) return;
    try {
      const response = await api("/train", { method: "POST" });
      notify(response.message || "Entrenamiento enviado");
    } catch (error) {
      notify(error.message);
    }
  }
          ...
          <aside class="panel clip-panel">
            ...
            <form class="manual-event-form stacked" on:submit|preventDefault={saveManualEvent}>
              <select bind:value={manualEventType} aria-label="Tipo de error">
                ...
                <option value="black_screen">Pantalla negra</option>
                <option value="buffering">Buffering</option>
                <option value="ad">Anuncio</option>
                <option value="repeated_ad">Anuncio repetido</option>
        ...
        <section class="panel">
          <div class="panel-head">
            ...
            <select bind:value={eventFilter} on:change={hydrate} aria-label="Filtro de evento">
              ...
              <option value="repeated_ad">Ad repetido</option>
              <option value="scene_change">Cambio de escena</option>
            </select>
          </div>
          <div class="timeline">
            {#if events.length}
              {#each events as event}
                <article class="event">
                  ...
                  <span class:critical={["freeze", "black_screen", "repeated_ad"].includes(event.type)} class:warn={["buffering", "scene_change"].includes(event.type)} class="badge">{event.type}</span>
                </article>
              {/each}
            {:else}
        ...
        <section class="admin-layout">
          {#if canConfigure}
            <div class="panel">
              ...
              <p>Solo admin puede configurar fuentes.</p>
